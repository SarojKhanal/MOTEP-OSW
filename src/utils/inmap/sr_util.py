...
def _download_inmap(_tmpdir, version):
    ...
    if ost == "Windows":
        _inmap_exe = Path(_tmpdir, "inmap_%s.exe" % version)
        _download("https://github.com/spatialmodel/inmap/releases/download/v%s/inmap-v%s-windows-%s.exe" %
                  (version, version, arch), _inmap_exe)
    elif ost == "Darwin":
        _inmap_exe = Path(_tmpdir, "inmap_%s" % version)
        _download("https://github.com/spatialmodel/inmap/releases/download/v%s/inmap-v%s-darwin-%s" %
                  (version, version, arch), _inmap_exe)
    elif ost == "Linux":
        _inmap_exe = Path(_tmpdir, "inmap_%s" % version)
        _download("https://github.com/spatialmodel/inmap/releases/download/v%s/inmap-v%s-linux-%s" %
                  (version, version, arch), _inmap_exe)
    else:
...
inMap_version = "1.9.0"
_tmpdir = Path(Path.cwd(), 'temp')
_tmpdir.mkdir(parents=True, exist_ok=True)
_inmap_exe = _download_inmap(_tmpdir=_tmpdir, version=inMap_version)
...
def run_sr(emis, model, output_variables, emis_units="tons/year"):
    ...
    if model not in model_paths.keys():
        ...
        msg = 'model must be one of \{{!s}\}, but is `{!s}`'.format(
            models, model)
    ...
    emis_file = Path(_tmpdir, "%s.shp" % (job_name))
    emis.to_file(emis_file)

    if _inmap_exe is None:
        _inmap_exe = _download_inmap(_tmpdir=_tmpdir, version=inMap_version)
    ...
    while True:
        try:
            status = subprocess.check_output(
                [_inmap_exe, "cloud", "status", "--job_name=%s" % job_name]).decode("utf-8").strip()
            print("simulation %s (%.0f seconds)" %
                  (status, time.time() - start), end='\r')
            if status == "Complete":
                break
            elif status != "Running":
    ...
    subprocess.check_call(
        [_inmap_exe, "cloud", "output", "--job_name=%s" % job_name])
    output = gpd.read_file("%s/OutputFile.shp" % job_name)

    shutil.rmtree(job_name)
    subprocess.check_call(
        [_inmap_exe, "cloud", "delete", "--job_name=%s" % job_name])
