...
# result_dir = '/Users/skhanal/Globus_local/MOTEP-OSW/outputs/paper_draft/SO'
# data_dir = "/Users/skhanal/Globus_local/MOTEP-OSW/inputs/EPA_PJM9Zones_osw2_17_N5X5R"
...
df_line_upgrade_decision = pd.read_csv(Path(result_dir, "line_upgrade_decision_size.csv"), header=None).head(n_lines_existing)
df = df_line_upgrade_decision.copy()
# find the indices of all elements that are equal to 1
indices = df.where(df > 0).stack().reset_index()
indices['type'] = "0"
indices['epoch'] = indices['level_1'] + 1
indices['s_max'] = indices[0].round()
...
df_lines_sliced = pd.merge(df_lines, indices[["index", "epoch", "type", 0]], how="left", on="index")
df_lines_sliced = df_lines_sliced[~df_lines_sliced["epoch"].isna()]
df_lines_sliced["epoch"] = df_lines_sliced["epoch"].astype(int)
df_lines_sliced["s_max"] = np.where(df_lines_sliced['type'].isin(['1', '2', '3']), df_lines_sliced['type'].replace({"1": 400, "2": 1400, "3": 2200}), df_lines_sliced[0])
df_lines_sliced.drop(columns=[0], inplace=True)
...
df_lines_existing_placeholder["type"] = pd.Series(['-1', '-1', '-1' , '0', '0'])
...
df_lines_sliced['Line Type'] = np.where(df_lines_sliced['type'].isin(['-1', '1', '2', '3']),
                                        df_lines_sliced['Line Type'] + ' ' + df_lines_sliced['s_max'].astype(str) + " MW",
                                        df_lines_sliced['Line Type'] + " MW")
