...
# result_dir = '/Users/skhanal/Globus_local/MOTEP-OSW/outputs/paper_draft/MO AQx1'
...
def make_spatial_transmission_plot(
    ...
    inputs:
        ...
        dt_lines is a pd.Dataframe with the following cols:
            name (str); from (str); to (str); mw (float), existing (bool), line_type (str)
    ...
    # sort data
    dt_zones.sort_values("name").reset_index(drop=True)
    dt_lines.sort_values("line_type").reset_index(drop=True)
    ...
    dt_zones["zone_color"] = [
        mpl.cm.Pastel2(j) for j in dt_zones["name"].factorize()[0]
    ]
    ...
    dt_lines = dt_lines.merge(
        dt_zones[["name", "centroid"]].rename(
            {"name": "from", "centroid": "from_centroid"}, axis=1
        ),
        how="left",
        on="from",
    )
    dt_lines = dt_lines.merge(
        dt_zones[["name", "centroid"]].rename(
            {"name": "to", "centroid": "to_centroid"}, axis=1
        ),
        how="left",
        on="to",
    )
    ...
    for j in dt_lines.index:
        from_temp = (
            dt_lines.loc[j, "from_centroid"].coords[0][0],
            dt_lines.loc[j, "to_centroid"].coords[0][0],
        )
        to_temp = (
            dt_lines.loc[j, "from_centroid"].coords[0][1],
            dt_lines.loc[j, "to_centroid"].coords[0][1],
        )
    ...
    if np.any(dt_zones.focus):
        ...
        for j in dt_lines_focus.index:
            from_temp = (
                dt_lines_focus.loc[j, "from_centroid"].coords[0][0],
                dt_lines_focus.loc[j, "to_centroid"].coords[0][0],
            )
            to_temp = (
                dt_lines_focus.loc[j, "from_centroid"].coords[0][1],
                dt_lines_focus.loc[j, "to_centroid"].coords[0][1],
            )
